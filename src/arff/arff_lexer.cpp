...
ArffToken ArffLexer::next_token(bool previous) {
    if (!previous)
        read_next_token();
    return next_token_;
}

void ArffLexer::read_next_token() {
    // in case the current character is a '}'
    if(m_b_close) {
        m_b_close = false;
        next_token_ = ArffToken("}", BRKT_CLOSE);
        return;
    }
    m_scanner->next();
    _skip_spaces();
    ...
    else if(icompare(str, "hierarchical")) {
        token = HIERARCHICAL;
    }
    else if(icompare(str, "?")) {
        token = MISSING_TOKEN;
    ...
    next_token_ = ArffToken(str, token);
}
