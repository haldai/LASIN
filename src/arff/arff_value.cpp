...
std::string arff_value2str(ArffValueEnum e) {
    ...
    case H_NOMINAL: return "H_NOMINAL";
...
ArffValue::ArffValue(const std::vector<std::string>& vec): m_int(0),
                                                           m_float(0.0f),
                                                           m_str(""),
                                                           m_type(H_NOMINAL),
                                                           m_vec(),
...
void ArffValue::set(const std::vector<std::string>& vec) {
    m_type = H_NOMINAL;
    m_vec = vec;
}
...
ArffValue::operator double() const {
...
ArffValue::operator std::vector<std::string>() const {
    switch(m_type) {
    case H_NOMINAL:
        return m_vec;
    default:
        THROW("operator std::vector<std::string> cannot work on type '%s'!",
              arff_value2str(m_type).c_str());
        return std::vector<std::string>();  // I need to keep the compiler happy!
    }
}
...
bool ArffValue::operator ==(const std::vector<std::string>& right) const {
    if (m_type != H_NOMINAL)
        return false;
    if (right.size() != m_vec.size())
        return false;
    else {
        for (std::size_t i = 0; i < right.size(); ++i) {
            if (m_vec[i].compare(right[i]) != 0) {
                return false;
            }
        }
    }
...
bool operator ==(const std::vector<std::string>& left, const ArffValue& right) {
    return (right == left);
}
