...
mnist_gen_masks(Data, Model, Thresh, Last_Centers, New_Centers, Masks, BG):-
    ...
    mnist_merge_mask_centers(Data, Mask_centers, Last_Centers, New_Centers, 0, BG),
    write('mask merge OK!'), nl,
    mnist_create_masks(New_Centers, Masks).

% merge old mask centers with new centers
mnist_merge_mask_centers(_, [], [], [], _, _):-
    !.
mnist_merge_mask_centers(Data, [C | Centers], [], [NC | New_Centers], T, BG):-
    write('merging mask '), write(T), nl,
    mnist_merge_mask(Data, C, [], NC, T, BG),
    T1 is T + 1,
    mnist_merge_mask_centers(Data, Centers, [], New_Centers, T1, BG),
    !.
mnist_merge_mask_centers(Data, [C | Centers], [OC | Old_Centers], [NC | New_Centers], T, BG):-
    write('merging mask '), write(T), nl,
    mnist_merge_mask(Data, C, OC, NC, T, BG),
    T1 is T + 1,
    mnist_merge_mask_centers(Data, Centers, Old_Centers, New_Centers, T1, BG).
...
%    kmeans(M2, 2, M),
%    !.
mnist_merge_mask(Data, C, M0, M, T, kmeans):-
    append_lists(M0, M1),
    append(M1, C, M2),
    list_to_set(M2, M3),
...
mnist_merge_mask(Data, C, M0, M, T, stroke):-
    append_lists(M0, M1),
    append(M1, C, M2),
    list_to_set(M2, M3),
    get_instance(Data, T, Inst),
...
mnist_merge_mask(Data, C, M0, M, T, spectral):-
    append_lists(M0, M1),
    append(M1, C, M2),
    list_to_set(M2, M3),
    % get_instance(Data, T, Inst),
